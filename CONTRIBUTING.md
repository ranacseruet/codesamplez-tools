...
- Visual baseline publishing and PR visual diffs are CI-managed workflows in this repository rather than local npm scripts. The per-PR visual diff is a `visual-diff` job in the main CI workflow gated on `test-and-build`, so it only runs after typecheck, tests, audit, budget, and build pass. After every successful `main` build, the baseline job rebuilds without tool selection and captures all configured route/viewport identities; never pass route scoping to baseline publication. Use the `PR Visual Diff (manual)` `workflow_dispatch` workflow to force a diff on demand.
