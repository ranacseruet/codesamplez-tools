...
- Root-level assets such as `index.html`, `robots.txt`, `sitemap.xml`, `og-home.png`, and `ads.txt` (when AdSense is configured) are copied through the root-assets path because they are not inside a tool directory.
...
- **`<title>`** carries an SEO/CTR variant distinct from the on-page `<h1>` and `og:title` (which stay the plain page name). Tool pages render `<Tool> – Free Online Tool | CodeSamplez`; the landing page renders `<Page title> | CodeSamplez`. The brand suffix comes from `organization.name` in `config/tooling-root.json`. The format lives in `formatToolDocumentTitle` / `formatRootDocumentTitle` (`scripts/document-helpers.js`).
...
- **Structured data (JSON-LD)** is assembled in `scripts/structured-data.js`:
  ...
  - Tool pages add `WebPage` + `WebApplication` + `BreadcrumbList` (+ `FAQPage` when FAQ items exist); the landing page adds `CollectionPage` + `ItemList` over all tools.
