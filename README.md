...
- **Production only (fail-closed).** Injection is enabled **only** when `NODE_ENV=production` (`npm run build` sets this via `scripts/build-tools.js`; CI sets it explicitly). Every other environment — `development`, `test`, CI, staging, or an unset `NODE_ENV` (bare `webpack serve`, IDE launchers) — resolves to null IDs, so non-production builds never load trackers or ads. Both local paths fail loudly on any regression: `npm run dev` refuses to compile tracker-bearing HTML (dev-server guard), and `npm run build:dev` fails the build if tracker markup appears in the emitted output (post-build scan).
- **Kill-switch.** `CST_DISABLE_ANALYTICS=1` (also `true`/`yes`) disables injection even for production builds — useful when driving real browsers against production HTML in automated QA.
- **QA never phones home.** The Playwright QA scripts (`qa:a11y:foundation`, `qa:cross-browser`) abort Google tracker/ad requests at the browser-context level, so runs against the production build don't pollute the live GA4/AdSense properties with localhost hits. Opt out with `QA_ALLOW_TRACKERS=1` when deliberately verifying tracker loading.
- **`npm start` serves production artifacts.** It hosts `build/` as-is, so a production build served locally still contains (and fires) trackers. That is expected — use `npm run dev` for tracker-free local development.
